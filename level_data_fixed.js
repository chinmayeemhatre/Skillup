...
app.get('/api/protected', authenticateToken, (req, res) => {
  res.json({ message: "Welcome authenticated user!", user: req.user });
        ...
        {
          title: "🗄️ Database Modeling & Optimization",
          content: "In NoSQL (MongoDB), data modeling is driven by your access patterns. <br>• <strong>Embedding vs Referencing:</strong> Embedding is faster for reads (one query), Referencing is better for large, frequently updated data.<br>• <strong>Indexing:</strong> Creating indexes on fields used in frequent queries (like email or username) can speed up searches from O(n) to O(log n).",
...
const Post = mongoose.model('Post', postSchema);`
        },
        {
          ...
          content: "Professional code is tested. <br>• <strong>Unit Testing:</strong> Testing individual functions in isolation (using Jest/Mocha).<br>• <strong>Integration Testing:</strong> Testing how multiple modules work together (API endpoints).<br>• <strong>Linting:</strong> Using ESLint to enforce consistent code style and catch bugs early.",
          code: `
...
- **[E-Commerce API](link):** RESTful API with Stripe payment integration.`
        ...
        {
          title: "🌐 Building a Stunning Personal Portfolio",
          content: "A portfolio website is your 'living resume'. It should showcase your personality and technical skills. <br>• <strong>Tech Stack:</strong> Use Next.js or Astro for speed and SEO. <br>• <strong>Must-Haves:</strong> An 'About Me' section, a clean 'Projects' grid with live links/source code, and a clear 'Contact' method. <br>• <strong>Performance:</strong> Ensure it scores 90+ on Lighthouse; developers appreciate fast sites.",
          code: `Portfolio MVP Structure:
- Home: Catchy headline + your photo.
- Projects: 3-4 deep dives (Title, Tech, Problem solved, Link).
- Blog: (Optional) Write about what you learn to show expertise.
- Contact: Resume PDF + LinkedIn + GitHub links.`
        },
        {
          title: "🌱 Open Source Contribution",
          content: "Contributing to open source proves you can work on large, production-grade codebases. <br>• <strong>Finding Projects:</strong> Use sites like 'Good First Issue' or 'First Timers Only'. <br>• <strong>The Process:</strong> Fork the repo -> Create a branch -> Fix a small bug/doc -> Submit a Pull Request. <br>• <strong>Impact:</strong> Even a documentation fix in a popular repo like React or VS Code looks great on your profile.",
          code: `Git Flow for Contributions:
1. fork origin/main
2. git checkout -b fix/typo-in-readme
3. git commit -m "docs: fix typo in installation guide"
4. git push origin fix/typo-in-readme
5. Open PR on original repository.`
        }
      ],
      resources: [
  ...
  5: {
    ...
    theory: {
      sections: [
        {
          title: "🗣️ Behavioral Interviews & Conflict Resolution",
          content: "Companies test your emotional intelligence (EQ) and conflict resolution skills. Beyond the <strong style='color:var(--yellow-l)'>STAR method</strong>, focus on showing growth. <br>• <strong>Adaptability:</strong> 'Tell me about a time you had to change your approach mid-project.'<br>• <strong>Leadership:</strong> 'Tell me about a time you mentored a junior teammate.'<br>• <strong>Handling Failure:</strong> Be honest about a mistake, focus on what you learned and how you never repeated it.",
          code: `STAR Method Refined:
1. Situation: High stakes, tight deadline.
2. Task: Migrating 1 million records with zero downtime.
3. Action: I didn't just code; I coordinated with DevOps, wrote a rollback script, and did a staging dry-run.
4. Result: Migration finished in 4 hours with 0% data loss.

Key Tip: Use "I" for your actions, but "We" for the team's success.`
        },
        {
          title: "💻 Master DSA: Advanced Patterns",
          content: "To clear interviews at FAANG+, you need more than just arrays. <br>• <strong>Dynamic Programming (DP):</strong> Solving subproblems to optimize (Memoization/Tabulation).<br>• <strong>Graph Algorithms:</strong> BFS/DFS, Dijkstra's (Shortest Path), Topological Sort.<br>• <strong>Sliding Window & Two Pointers:</strong> Essential for string/array optimization from O(n^2) to O(n).",
          code: `
...
`,
        ...
        {
          title: "🏗️ System Design: Scalability & Performance",
          content: "System design is about making trade-offs. <br>• <strong>Load Balancers:</strong> Use Nginx or AWS ELB to avoid single points of failure.<br>• <strong>Database Sharding:</strong> Splitting a large database into smaller, faster pieces.<br>• <strong>Message Queues:</strong> Using RabbitMQ or Kafka for asynchronous tasks (e.g., sending emails) to prevent blocking the main request.",
          code: `Asynchronous Task Workflow:
1. User clicks "Sign Up".
2. Web Server creates user in DB.
3. Web Server sends a "Send Welcome Email" message to RabbitMQ.
4. Web Server immediately returns "Success" to User.
5. Background Worker picks message from Queue and sends email.

Benefit: The user doesn't wait 5 seconds for the email server to respond.`
        }
      ],
      resources: [
...
| StartupX | Frontend   | Oct 15, 2024 | startupx.io   | Emailed CEO        | Rejected    | Ask for feedback |`
  ...
  7: {
    ...
    theory: {
      sections: [
        {
          title: "🎉 Evaluating & Negotiating Offers",
          content: "An offer is more than just base salary. <strong style='color:var(--yellow-l)'>Total Compensation (TC)</strong> includes Base, Bonus, and Equity (RSUs/Options). Beyond money, evaluate the <strong style='color:var(--yellow-l)'>Engineering Culture</strong>: Do they have good testing practices? Is there a mentorship program? What is the tech stack? Never accept immediately; always express gratitude and ask for 48-72 hours to review the formal document.",
          code: `Salary Negotiation Email Template:
"Hi [Recruiter Name], thank you so much for the offer! I am incredibly excited about the prospect of joining [Company].

Based on my research and other ongoing interviews, I was expecting a base salary closer to [Amount]. Given my experience in [Specific Skill], is there any flexibility in the budget for this role?"

Tip: Negotiation is a collaboration, not a confrontation.`
        },
        {
          title: "🚀 Professional Onboarding & First 30 Days",
          content: "Your goal in the first month is to <strong style='color:var(--yellow-l)'>Absorb and Assist</strong>. <br>• <strong>The First Week:</strong> Focus on setting up your environment and understanding the business domain.<br>• <strong>Week 2-4:</strong> Pick up 'Good First Issues' (small bugs, doc fixes) to get your first PR merged. <br>• <strong>Communication:</strong> Over-communicate your progress in Daily Standups.",
          code: `Effective Daily Standup Format:
1. Yesterday: Setup the local dev environment and fixed a CSS alignment bug in the login page.
2. Today: I'll start working on the User Profile API endpoint logic.
3. Blockers: I'm waiting for access to the staging database credentials.

Tip: Be concise and stick to the 3-point format.`
        },
        {
          title: "🤝 Corporate Communication & Culture",
          content: "Professionalism is about reliable delivery and clear communication. <br>• <strong>Slack/Teams:</strong> Keep channels clean, use threads for discussions.<br>• <strong>Jira/Trello:</strong> Keep your tickets updated—it's your manager's window into your work.<br>• <strong>Meeting Etiquette:</strong> Be on time, have your camera on (if remote), and take notes.",
          code: `Slack Professional Etiquette:
- Don't just say "Hi". State your question/update immediately.
- Use threads to keep main channels quiet.
- Use emojis for quick acknowledgments (👀 for looking, ✅ for done).`
        }
      ],
      resources: [
